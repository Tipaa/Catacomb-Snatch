...
public class Downloader {
    ...
    private static String[][] binFiles = {
	    { "lwjgl.jar", "a9deb51bea77db85de26f7b2eb1ecbea" },
    ...
    private static String[][] nativeFiles = {
	    { "linux_native.jar", "6e05a97164478a1c0428179d78dc47ea" },
    ...
    private static String[][][] nativeLocalFiles = {
	    // linux
	    {
		    { "libjinput-linux64.so",
			    "23a6b611eaab617a9394f932b69ae034" },
    ...
    private static String[][] soundFiles = {
	    { "Background 1.ogg", "dc981f1c6abbc9cbd789316c13404d39" },
    ...
    private static void showAllMD5s() {
	File binDir = getBinDir();
	File soundDir = Downloader.getSoundDir();
	File nativeDir = Downloader.getNativeDir();
	for (String[] s : binFiles) {
	    try {
		System.out.println(s[0]
			+ ":"
			+ getMD5Checksum(new File(binDir, s[0])
				.getAbsolutePath()));
	    } catch (Exception e) {
		e.printStackTrace();
	    }
	}
	for (String[] s : nativeFiles) {
	    try {
		System.out.println(s[0]
			+ ":"
			+ getMD5Checksum(new File(nativeDir, s[0])
				.getAbsolutePath()));
	    } catch (Exception e) {
		e.printStackTrace();
	    }
	...
	for (String[] s : soundFiles) {
		...
		System.out.println(s[0]
			+ ":"
			+ getMD5Checksum(new File(soundDir, s[0])
				.getAbsolutePath()));
	    } catch (Exception e) {
	    }
	}
	...
	if (children != null) {
		...
		for (int j = 0; j < found.size(); j++) {
			...
			String hash = "";
			try {
			    hash = getMD5Checksum(new File(binDir, binFile)
				    .getAbsolutePath());
			} catch (Exception e) {
	...
	if (children != null) {
	    ...
	    //
	    int osId = EnumOSMappingHelper.enumOSMappingArray[MojamComponent
		    .getOs().ordinal()];
	    if (osId < 5 && osId > 0) {
		// now check the missing files..
		boolean notfoundorok = false;
		for (int i = 0; i < nativeLocalFiles[osId - 1].length; i++) {
		    String nativeFile = nativeLocalFiles[osId - 1][i][0]
			    .toString();
			...
			if (nativeFile.equals(found.get(j).toString())) {
			    ...
			    String hash = "";
			    try {
				hash = getMD5Checksum(new File(nativeDir,
	...
	final String nativeLibDir = MojamComponent.getMojamDir()
		...
		+ "native"
		+ File.separator;
	final String binDir = getBinDir().getAbsolutePath().toString()
		+ File.separator;
	String jarFile = "";
	switch (EnumOSMappingHelper.enumOSMappingArray[MojamComponent.getOs()
		.ordinal()]) {
	case 1: // '\001'
		// linux
	...
	if (!jarFile.isEmpty()) {
		...
		try {
		    hash = getMD5Checksum(new File(binDir, jarFile)
			    .getAbsolutePath());
		} catch (Exception e) {
		    // TODO Auto-generated catch block
		    e.printStackTrace();
		...
		if (!md5.equals(hash)) {
		    if (!download(baseURL + "native/" + jarFile, binDir
			    + jarFile)) {
			return;
		    }
		}
	...
	if (children != null) {
		...
		for (int j = 0; j < found.size(); j++) {
			...
			String hash = "";
			try {
			    hash = getMD5Checksum(new File(soundDir, soundFile)
				    .getAbsolutePath());
			} catch (Exception e) {
			    // TODO Auto-generated catch block
			    e.printStackTrace();
	...
	return new File(new File(MojamComponent.getMojamDir(), "resources"),
		"sound");
    }
	...
	try {
		...
		byte[] buffer = new byte[1 << Options.getAsInteger(
			Options.DLBUFFERSIZE, 13)];
